...
public class ClaimsScreen {
    // стрница Заявки и расскрытая заявка
    public ViewInteraction claimsScreenName = onView(withText("Заявки"));
    ...
    public ViewInteraction takeToWorkClaim = onView(withText("В работу"));
    public ViewInteraction cancelClaim = onView(withText("Отменить"));
    public ViewInteraction throwOffClaim = onView(withText("Сбросить"));
    public ViewInteraction toExecuteClaim = onView(withText("Исполнить"));
    ...
    public ViewInteraction editComment(int index) {
        return onView(allOf(withIndex(withId(R.id.edit_comment_image_button), index),
                withContentDescription("кнопка редактирования комментария")));
    }
}